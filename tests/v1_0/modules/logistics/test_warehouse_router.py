...
async def test_create_warehouse_endpoint(async_client: AsyncClient):
    node_id = uuid.uuid4()
    ...
    with patch("app.v1_0.modules.logistics.service.LogisticsNodeService.create_node", new_callable=AsyncMock) as mock_create:
        ...
        response = await async_client.post("/api/v1/warehouses/", json={
            "name": "API Test Warehouse",
            "address": "123 Street",
            "city": "Bogota",
            "country": "Colombia"
        ...
        data = response.json()
        assert data["name"] == "API Test Warehouse"
        assert data["id"] == str(node_id)
        assert data["continent"] == "SOUTH AMERICA"


async def test_get_warehouse_not_found(async_client: AsyncClient):
    ...
    with patch("app.v1_0.modules.logistics.service.LogisticsNodeService.get_node", new_callable=AsyncMock) as mock_get:
        ...
        response = await async_client.get(f"/api/v1/warehouses/{uuid.uuid4()}")
        assert response.status_code == 404


async def test_patch_warehouse_endpoint(async_client: AsyncClient):
    node_id = uuid.uuid4()
    from datetime import datetime, timezone
    now = datetime.now(timezone.utc)
    ...
    with patch("app.v1_0.modules.logistics.service.LogisticsNodeService.update_node", new_callable=AsyncMock) as mock_update:
        ...
        response = await async_client.patch(f"/api/v1/warehouses/{node_id}", json={
            "name": "Updated Warehouse"
        })
...
async def test_list_warehouses_endpoint(async_client: AsyncClient):
    ...
    with patch("app.v1_0.modules.logistics.service.LogisticsNodeService.list_nodes", new_callable=AsyncMock) as mock_list:
        ...
        response = await async_client.get("/api/v1/warehouses/?continent=SOUTH AMERICA&country=Colombia")
...
async def test_delete_warehouse_endpoint(async_client: AsyncClient):
    with patch("app.v1_0.modules.logistics.service.LogisticsNodeService.delete_node", new_callable=AsyncMock) as mock_delete:
        ...
        response = await async_client.delete(f"/api/v1/warehouses/{uuid.uuid4()}")
